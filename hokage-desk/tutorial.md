...
Bem-vindo ao Hokage Desk! Este tutorial irá guiá-lo através do processo de criação de uma aplicação Angular com um backend alimentado por IA usando o Genkit da Google.
...
4.  Cria o `missionGeneratorFlow`, que recebe o nome de um ninja como entrada e usa o modelo `gemini-2.5-flash` para gerar uma lista de missões em formato JSON.
...
2.  Expõe um `loading` para sinalizar se a api terminou a requisição.
...
Acesse `http://localhost:4200` em seu navegador, digite a descrição da missao e veja a IA em ação!
